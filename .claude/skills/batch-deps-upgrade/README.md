...
It discovers the open Dependabot PRs and Semgrep tickets, applies the upgrades to `pyproject.toml`, runs `poetry lock` and `poetry install`, validates with lint, type-check, unit tests, integration tests, and faucet tests across all CI Python versions, and writes `code-changes.md`, `pr-description.md` and `close-list.md` plus a commit message. It creates the branch but does not commit, and writes nothing outside the repo.
