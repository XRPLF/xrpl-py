...
class TestAccount(IsolatedAsyncioTestCase):
    async def test_does_account_exist_true(self):
        self.assertTrue(
            await does_account_exist(WALLET.classic_address, ASYNC_JSON_RPC_CLIENT)
        )

    async def test_does_account_exist_false(self):
        address = "rG1QQv2nh2gr7RCZ1P8YYcBUcCCN633jCn"
        self.assertFalse(await does_account_exist(address, ASYNC_JSON_RPC_CLIENT))

    async def test_does_account_exist_xaddress(self):
        xaddress = classic_address_to_xaddress(WALLET.classic_address, None, True)
        self.assertTrue(await does_account_exist(xaddress, ASYNC_JSON_RPC_CLIENT))

    async def test_get_balance(self):
        self.assertEqual(
            await get_balance(NEW_WALLET.classic_address, ASYNC_JSON_RPC_CLIENT),
            1000000000,
        )

    async def test_get_account_transactions(self):
        transactions = await get_account_transactions(
            NEW_WALLET.classic_address, ASYNC_JSON_RPC_CLIENT
        )
        self.assertEqual(len(transactions), 1)
        self.assertEqual(transactions[0]["tx"]["TransactionType"], "Payment")
        self.assertEqual(transactions[0]["tx"]["Amount"], "1000000000")

    async def test_get_account_transactions_empty(self):
        transactions = await get_account_transactions(
            EMPTY_WALLET.classic_address, ASYNC_JSON_RPC_CLIENT
        )
    ...
    async def test_payment_transactions_xaddress(self):
        xaddress = classic_address_to_xaddress(NEW_WALLET.classic_address, None, True)
        transactions = await get_account_transactions(xaddress, ASYNC_JSON_RPC_CLIENT)
        self.assertEqual(len(transactions), 1)
        self.assertEqual(transactions[0]["tx"]["TransactionType"], "Payment")
        self.assertEqual(transactions[0]["tx"]["Amount"], "1000000000")

    async def test_get_latest_transaction(self):
    ...
    async def test_get_account_info(self):
        response = await get_account_info(WALLET.classic_address, ASYNC_JSON_RPC_CLIENT)
