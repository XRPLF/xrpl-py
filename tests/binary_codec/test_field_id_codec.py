...
class TestFieldIDCodec(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data_driven_fixtures = fixtures_utils.get_data_driven_fixtures()

    def test_encode(self):
        for test in self.data_driven_fixtures.field_tests:
            # .hex().upper() just formats the resulting bytes for comparison with upper case hex string in fixture data
            self.assertEqual(test.expected_hex, self.field_id_codec.encode(test.name).hex().upper())

    def test_decode(self):
        for test in self.data_driven_fixtures.field_tests:
            self.assertEqual(test.name, self.field_id_codec.decode(test.expected_hex))
    ...
    def test_encode_field_id_1_byte(self):
        ...
        field_id = self.field_id_codec._encode_field_id(field_header)
    ...
    def test_decode_field_id_1_byte(self):
        ...
        field_header = self.field_id_codec._decode_field_id(field_id)
