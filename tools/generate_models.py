...
# Gets all `KnownFormats`-style formats (for transactions and ledger objects)
# And also processes comments
# e.g.
...
_REGEX = (
    r"^ *add\(jss::([^\"\n, ]+),[ \n]*[tl]t[A-Z_]+,[ \n]*{[ \n]*((({sf[A-Za-z0-9]+, *"
    r"soe(OPTIONAL|REQUIRED|DEFAULT)},? *)?(//[A-Za-z ]+)?[ \n]*)*)},[ \n]*[pseudocC]+ommonFields\);"
)
...
# Converts a line of the format
#        {sfFirstLedgerSequence,  soeOPTIONAL},
# To a tuple of the format ("sfFirstLedgerSequence", "soeOPTIONAL")
def _format_cpp_output(raw_tx_format: str) -> List[Tuple[str, ...]]:
    return [
        tuple([part.strip() for part in element.strip(" \n,{}").split(", ")])
        for element in raw_tx_format.strip().split("\n")
        if not element.strip().startswith("//")
    ]


# Parses rippled source code into a more machine-readable format
def _parse_rippled_source(
    ...
    tx_formats_hits = re.findall(_REGEX, tx_formats_cpp, re.MULTILINE)
    tx_formats = {hit[0]: _format_cpp_output(hit[1]) for hit in tx_formats_hits}

    # Get LedgerFormats
    ledger_formats_cpp = _read_file(
        os.path.join(folder, "src/ripple/protocol/impl/LedgerFormats.cpp")
    )
    ledger_formats_hits = re.findall(_REGEX, ledger_formats_cpp, re.MULTILINE)
    ledger_formats = {hit[0]: _format_cpp_output(hit[1]) for hit in ledger_formats_hits}

    return sfields, tx_formats, ledger_formats


# rippled serialized types mapped to Python types
TYPE_MAP = {
    "UINT8": "int",
    "UINT16": "int",
    "UINT32": "int",
...
# import locations for types that need to be imported
IMPORT_MAP = {
...
# Adds the imports to the __init__.py in the transaction model folder
def _update_init_file(folder: str, tx: str, name: str) -> None:
    filename = f"xrpl/models/{folder}/__init__.py"
    with open(filename) as f:
        index_file = f.read()
    index_file = index_file.replace(
        "\n\n__all__ = [",
        f"from xrpl.models.{folder}.{name} import {tx}" + "\n\n__all__ = [",
    )
    index_file = index_file.replace(
        "\n]",
        f'\n    "{tx}",\n]',
...
# Does the main processing for transaction models
def _tx_main(
...
"""
        ...
        _update_init_file("transactions", tx, name)

        print("Added " + tx)


# Does the main processing for ledger object models
def _ledger_main(
    sfields: Dict[str, List[str]], ledger_formats: Dict[str, List[Tuple[str, ...]]]
) -> None:
    objects_to_add = []
    existing_library_objects = {m.value for m in LedgerEntryType}
    for object in ledger_formats:
        if object not in existing_library_objects:
            objects_to_add.append((object, _key_to_json(object)))

    with open("xrpl/models/ledger_objects/ledger_entry_type.py", "a") as f:
        for object, name in objects_to_add:
            f.write(f'    {name.upper()} = "{object}"\n')

    for object, name in objects_to_add:
        object_format = ledger_formats[object]
        ...
        param_lines = [
            _generate_param_line(param[0], param[1] == "soeREQUIRED")
            for param in sorted(object_format, key=lambda x: x[0])
            if param != ("",)
        ]
        param_lines.sort(key=lambda x: "REQUIRED" not in x)
        params = "\n".join(param_lines)
        model = f"""@require_kwargs_on_init
@dataclass(frozen=True)
class {object}(LedgerObject):
    \"\"\"Represents a {object} ledger object.\"\"\"

{params}

    ledger_entry_type: LedgerEntryType = field(
        default=LedgerEntryType.{name.upper()},
        init=False,
    )
"""
        ...
        imported_models = f"""\"\"\"Model for {object} ledger object.\"\"\"
...
"""
        ...
        with open(f"xrpl/models/ledger_objects/{name}.py", "w+") as f:
            f.write(imported_models + "\n\n" + model)

        _update_init_file("ledger_objects", object, name)
...
if __name__ == "__main__":
    folder = sys.argv[1]
    sfields, tx_formats, ledger_formats = _parse_rippled_source(folder)
    _tx_main(sfields, tx_formats)
    _ledger_main(sfields, ledger_formats)
