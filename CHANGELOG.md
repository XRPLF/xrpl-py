...
- Support for `Dynamic Multi-Purpose Tokens` (XLS-94d): `MPTokenIssuanceCreate` and `MPTokenIssuanceSet` accept `domain_id` and `immutable_flags`, and `MPTokenIssuanceSet` can additionally update `mptoken_metadata` / `transfer_fee` and enable MPT issuance capabilities via the `TF_MPT_SET_*` flags. Under DynamicMPT these fields and flags are mutable by default; a bit set in `ImmutableFlags` (see `MPTokenIssuanceImmutableFlag`) permanently makes the corresponding field or flag immutable.
- Support for `Confidential MPT` (XLS-96): the `ConfidentialMPTConvert`, `ConfidentialMPTConvertBack`, `ConfidentialMPTSend`, `ConfidentialMPTMergeInbox`, and `ConfidentialMPTClawback` transactions, plus the `MPTokenIssuanceCreate`/`MPTokenIssuanceSet` fields and flags for holding confidential balances (`tfMPTCanHoldConfidentialBalance` at creation, `tfMPTSetCanHoldConfidentialBalance` to enable it afterwards, `tifMPTCanHoldConfidentialBalance` to make it immutable, and the `issuer_encryption_key` / `auditor_encryption_key` fields). The optional native cryptography (proof generation and balance decryption) is provided by the separate `xrpl.ext.confidential` extension.
