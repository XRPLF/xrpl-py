...
class MPTokenIssuanceSet(Transaction):
    ...
    def _get_errors(self: Self) -> Dict[str, str]:
        ...
        has_issuer_key = self.issuer_encryption_key is not None
        has_auditor_key = self.auditor_encryption_key is not None

        if (
            self.issuer_encryption_key is not None
            and len(self.issuer_encryption_key) != ENCRYPTION_KEY_LENGTH
        ):
        ...
        if (
            self.auditor_encryption_key is not None
            and len(self.auditor_encryption_key) != ENCRYPTION_KEY_LENGTH
        ):
        ...
        if self.holder is not None and (has_issuer_key or has_auditor_key):
            errors["holder"] = (
