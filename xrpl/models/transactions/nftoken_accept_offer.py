...
class NFTokenAcceptOffer(Transaction):
    ...
    To indicate direct mode, use either the `nf_token_sell_offer` or
    `nf_token_buy_offer` fields, but not both. To indicate brokered mode,
    use both the `nf_token_sell_offer` and `nf_token_buy_offer` fields. If you use
    neither `nf_token_sell_offer` nor `nf_token_buy_offer`, the transaction is invalid.
    """
    ...
    def _get_errors(self: NFTokenAcceptOffer) -> Dict[str, str]:
        return {
            ...
            for key, value in {
                **super()._get_errors(),
                "nf_token_sell_offer": self._get_nftoken_sell_offer_error(),
                "nf_token_buy_offer": self._get_nftoken_buy_offer_error(),
    ...
    def _get_nftoken_sell_offer_error(self: NFTokenAcceptOffer) -> Optional[str]:
        if self.nf_token_broker_fee is not None and self.nf_token_sell_offer is None:
            return "Must be set if using brokered mode"
        if self.nf_token_sell_offer is None and self.nf_token_buy_offer is None:
            return "Must set either nf_token_buy_offer or nf_token_sell_offer"
        return None

    def _get_nftoken_buy_offer_error(self: NFTokenAcceptOffer) -> Optional[str]:
        if self.nf_token_broker_fee is not None and self.nf_token_buy_offer is None:
            return "Must be set if using brokered mode"
        if self.nf_token_sell_offer is None and self.nf_token_buy_offer is None:
            return "Must set either nf_token_buy_offer or nf_token_sell_offer"
        return None
