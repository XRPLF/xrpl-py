...
class AsyncWebsocketClient(AsyncClient):
    ...
    async def _handler(self: AsyncWebsocketClient) -> None:
        """
    ...
    def _set_up_future(self: AsyncWebsocketClient, request: Request) -> None:
        """
    ...
    async def _do_send(self: AsyncWebsocketClient, request: Request) -> None:
        assert self._websocket is not None  # mypy
    ...
    async def send(self: AsyncWebsocketClient, request: Request) -> None:
        ...
        Raises:
            XRPLWebsocketException: If there is already an open request by the
                request's ID, or if this AsyncWebsocketClient is not open.
        """
        if not self.is_open():
            raise XRPLWebsocketException("Websocket is not open")
    ...
    async def request_impl(self: AsyncWebsocketClient, request: Request) -> Response:
        """
