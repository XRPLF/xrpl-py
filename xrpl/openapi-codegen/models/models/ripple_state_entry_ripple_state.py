...
class RippleStateEntryRippleState(BaseModel):
    ...
    def _get_errors(self: RippleStateEntryRippleState) -> Dict[str, str]:
        ...
        if self.accounts is not None and len(self.accounts) < 2:
            errors["RippleStateEntryRippleState"] = (
                "Field `accounts` must have a length greater than or equal to 2"
            )
        if self.accounts is not None and len(self.accounts) > 2:
            errors["RippleStateEntryRippleState"] = (
                "Field `accounts` must have a length less than or equal to 2"
            )
        return errors
