...
class OracleSet(Transaction):
    ...
    def _get_errors(self: OracleSet) -> Dict[str, str]:
        ...
        if (
            ...
            and self.last_update_time <= 946684799
        ):
            errors["OracleSet"] = "last_update_time must be greater than 946684799"
        if self.provider is not None and len(self.provider) > 256:
            errors["OracleSet"] = "Field `provider` must have a length less than or equal to 256"
        if self.uri is not None and len(self.uri) > 256:
            errors["OracleSet"] = "Field `uri` must have a length less than or equal to 256"
        if self.asset_class is not None and len(self.asset_class) > 16:
            errors["OracleSet"] = "Field `asset_class` must have a length less than or equal to 16"
        if self.price_data_series is not None and len(self.price_data_series) < 1:
            errors["OracleSet"] = "Field `price_data_series` must have a length greater than or equal to 1"
        if self.price_data_series is not None and len(self.price_data_series) > 10:
            errors["OracleSet"] = "Field `price_data_series` must have a length less than or equal to 10"
        return errors
