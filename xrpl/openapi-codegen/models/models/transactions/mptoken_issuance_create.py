...
class MPTokenIssuanceCreate(Transaction):
    ...
    def _get_errors(self: MPTokenIssuanceCreate) -> Dict[str, str]:
        ...
        if (
            not self.has_flag(MPTokenIssuanceCreateFlag.TF_MPT_CAN_TRANSFER) and
            self.transfer_fee is not None
        ):
            errors["MPTokenIssuanceCreate"] = "`transfer_fee` must not be set without flag `TF_MPT_CAN_TRANSFER`"
        if self.transfer_fee is not None and self.transfer_fee < 0:
            errors["MPTokenIssuanceCreate"] = "Field `transfer_fee` must have a value greater than or equal to 0"
        if self.transfer_fee is not None and self.transfer_fee > 50000:
            errors["MPTokenIssuanceCreate"] = "Field `transfer_fee` must have a value less than or equal to 50000"
        if self.mp_token_metadata is not None and len(self.mp_token_metadata) < 1:
            errors["MPTokenIssuanceCreate"] = "Field `mp_token_metadata` must have a length greater than or equal to 1"
        return errors
