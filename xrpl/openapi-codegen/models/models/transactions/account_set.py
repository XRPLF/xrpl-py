...
class AccountSet(Transaction):
    ...
    def _get_errors(self: AccountSet) -> Dict[str, str]:
        ...
        if (
            self.has_flag(AccountSetFlag.ASF_AUTHORIZED_NFTOKEN_MINTER) and
            self.nftoken_minter is None
        ):
            errors["AccountSet"] = "`nftoken_minter` must be set with flag `ASF_AUTHORIZED_NFTOKEN_MINTER`"
        # This check is only applicable if the flag belongs to the `flags` field inherited from base Transaction.
        # For other cases such as `set_flag` or `clear_flag` field in account_info transaction, please fix accordingly.
        if (
            not self.has_flag(AccountSetFlag.ASF_AUTHORIZED_NFTOKEN_MINTER) and
            self.nftoken_minter is not None
        ):
            errors["AccountSet"] = "`nftoken_minter` must not be set without flag `ASF_AUTHORIZED_NFTOKEN_MINTER`"
        # This check is only applicable if the flag belongs to the `flags` field inherited from base Transaction.
        # For other cases such as `set_flag` or `clear_flag` field in account_info transaction, please fix accordingly.
        if (
            self.has_flag(AccountSetFlag.ASF_AUTHORIZED_NFTOKEN_MINTER) and
            ...
            errors["AccountSet"] = "`nftoken_minter` must not be set with flag `ASF_AUTHORIZED_NFTOKEN_MINTER`"
        ...
        if self.domain is not None and len(self.domain) > 256:
            errors["AccountSet"] = "Field `domain` must have a length less than or equal to 256"
        if self.transfer_rate is not None and self.transfer_rate != 0:
            if self.transfer_rate < 1000000000:
                errors["AccountSet"] = "Field `transfer_rate` must have a value greater than or equal to 1000000000"
            if self.transfer_rate > 2000000000:
                errors["AccountSet"] = "Field `transfer_rate` must have a value less than or equal to 2000000000"
        if self.tick_size is not None and self.tick_size != 0:
            if self.tick_size < 3:
                errors["AccountSet"] = "Field `tick_size` must have a value greater than or equal to 3"
            if self.tick_size > 15:
                errors["AccountSet"] = "Field `tick_size` must have a value less than or equal to 15"
        return errors
