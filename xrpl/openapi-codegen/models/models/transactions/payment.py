...
class Payment(Transaction):
    ...
    def _get_errors(self: Payment) -> Dict[str, str]:
        ...
        if (
            not self.has_flag(PaymentFlag.TF_PARTIAL_PAYMENT) and
            self.deliver_min is not None
        ):
            errors["Payment"] = "`deliver_min` must not be set without flag `TF_PARTIAL_PAYMENT`"
        return errors
