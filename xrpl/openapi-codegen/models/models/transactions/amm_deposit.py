...
class AMMDeposit(Transaction):
    ...
    def _get_errors(self: AMMDeposit) -> Dict[str, str]:
        ...
        if (
            self.amount2 is not None and
            self.amount is None
        ):
            errors["AMMDeposit"] = "Must set `amount` with `amount2.`"
        if (
            self.e_price is not None and
            self.amount is None
        ):
            errors["AMMDeposit"] = "Must set `amount` with `e_price.`"
        if (
            self.lp_token_out is None and
            self.amount is None
        ):
            errors["AMMDeposit"] = "At least one of `lp_token_out`, `amount` must be set."
        if self.trading_fee is not None and self.trading_fee < 0:
            errors["AMMDeposit"] = "Field `trading_fee` must have a value greater than or equal to 0"
        if self.trading_fee is not None and self.trading_fee > 1000:
            errors["AMMDeposit"] = "Field `trading_fee` must have a value less than or equal to 1000"
        return errors
