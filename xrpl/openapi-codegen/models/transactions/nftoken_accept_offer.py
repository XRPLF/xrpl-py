...
class NFTokenAcceptOffer(Transaction):
    ...
    transaction_type: TransactionType = field(
        default=TransactionType.NFTOKEN_ACCEPT_OFFER, init=False
    )
    ...
    def _get_errors(self: NFTokenAcceptOffer) -> Dict[str, str]:
        ...
        if (
            ...
            and self.nftoken_buy_offer is None
            and self.nftoken_sell_offer is None
        ):
            errors["NFTokenAcceptOffer"] = (
                "Must set `nftoken_buy_offer`, `nftoken_sell_offer` with `nftoken_broker_fee.`"
            )
        if self.nftoken_sell_offer is None and self.nftoken_buy_offer is None:
            errors["NFTokenAcceptOffer"] = (
                "At least one of `nftoken_sell_offer`, `nftoken_buy_offer` must be set."
            )
        if (
