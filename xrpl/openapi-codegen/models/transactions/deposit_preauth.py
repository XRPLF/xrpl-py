...
class DepositPreauth(Transaction):
    ...
    transaction_type: TransactionType = field(
        default=TransactionType.DEPOSIT_PREAUTH, init=False
    )
    ...
    def _get_errors(self: DepositPreauth) -> Dict[str, str]:
        ...
        if (
            self.authorize_credentials is not None
            and len(self.authorize_credentials) < 1
        ):
            errors["DepositPreauth"] = (
                "Field `authorize_credentials` must have a length greater than or equal to 1"
            )
        if (
            self.authorize_credentials is not None
            and len(self.authorize_credentials) > 8
        ):
            errors["DepositPreauth"] = (
                "Field `authorize_credentials` must have a length less than or equal to 8"
            )
        if len(self.authorize_credentials) != len(set(self.authorize_credentials)):
            errors["DepositPreauth"] = (
                "`authorize_credentials` list cannot contain duplicate values"
            )
        if (
            self.unauthorize_credentials is not None
            and len(self.unauthorize_credentials) < 1
        ):
            errors["DepositPreauth"] = (
                "Field `unauthorize_credentials` must have a length greater than or equal to 1"
            )
        if (
            self.unauthorize_credentials is not None
            and len(self.unauthorize_credentials) > 8
        ):
            errors["DepositPreauth"] = (
                "Field `unauthorize_credentials` must have a length less than or equal to 8"
            )
        if len(self.unauthorize_credentials) != len(set(self.unauthorize_credentials)):
            errors["DepositPreauth"] = (
                "`unauthorize_credentials` list cannot contain duplicate values"
            )
        return errors
