...
class NFTokenMint(Transaction):
    ...
    transaction_type: TransactionType = field(
        default=TransactionType.NFTOKEN_MINT, init=False
    )
    ...
    def _get_errors(self: NFTokenMint) -> Dict[str, str]:
        ...
        if self.has_flag(NFTokenMintFlag.TF_TRANSFERABLE) and self.transfer_fee is None:
            errors["NFTokenMint"] = (
                "`transfer_fee` must be set with flag `TF_TRANSFERABLE`"
            )
        ...
        if self.uri is not None and len(self.uri) > 512:
            errors["NFTokenMint"] = (
                "Field `uri` must have a length less than or equal to 512"
            )
        return errors
