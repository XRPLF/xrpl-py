...
class TicketCreate(Transaction):
    ...
    transaction_type: TransactionType = field(
        default=TransactionType.TICKET_CREATE, init=False
    )
    ...
    def _get_errors(self: TicketCreate) -> Dict[str, str]:
        ...
        if self.ticket_count is not None and self.ticket_count < 1:
            errors["TicketCreate"] = (
                "Field `ticket_count` must have a value greater than or equal to 1"
            )
        if self.ticket_count is not None and self.ticket_count > 250:
            errors["TicketCreate"] = (
                "Field `ticket_count` must have a value less than or equal to 250"
            )
        return errors
