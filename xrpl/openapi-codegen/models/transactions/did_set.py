...
class DIDSet(Transaction):
    ...
    transaction_type: TransactionType = field(
        default=TransactionType.DID_SET, init=False
    )
    ...
    def _get_errors(self: DIDSet) -> Dict[str, str]:
        ...
        if self.data is None and self.did_document is None and self.uri is None:
            errors["DIDSet"] = (
                "At least one of `data`, `did_document`, `uri` must be set."
            )
        if self.data is not None and len(self.data) > 256:
            errors["DIDSet"] = (
                "Field `data` must have a length less than or equal to 256"
            )
        if self.did_document is not None and len(self.did_document) > 256:
            errors["DIDSet"] = (
                "Field `did_document` must have a length less than or equal to 256"
            )
