...
class MPTokenIssuanceSet(Transaction):
    ...
    transaction_type: TransactionType = field(
        default=TransactionType.MPTOKEN_ISSUANCE_SET, init=False
    )
    ...
    def _get_errors(self: MPTokenIssuanceSet) -> Dict[str, str]:
        ...
        if self.has_flag(MPTokenIssuanceSetFlag.TF_MPT_LOCK) and self.has_flag(
            ...
            errors["MPTokenIssuanceSet"] = (
                "flag conflict: only of these flags `TF_MPT_LOCK`, `TF_MPT_UNLOCK` can be set"
            )
        return errors
