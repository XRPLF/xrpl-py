...
# Platform-gated FFI surface: these secp256k1 point-level symbols are exported by
# the Linux/macOS shared library but not by the Windows DLL (statically-linked
# secp256k1 symbols aren't re-exported on Windows). Declaring them only off-Windows
# lets the Windows extension link, while the features that need them stay available
# on Linux/macOS:
#   * secp256k1_ec_pubkey_parse -> commitments.create_bulletproof /
#     verify_bulletproof (guarded on hasattr(lib, ...) at runtime).
#   * secp256k1_ec_pubkey_serialize + secp256k1_elgamal_add / _subtract ->
#     homomorphic.py, which predicts a confidential balance's next state
#     (Enc(a) +/- Enc(b) = Enc(a +/- b)) for chaining proofs across multiple
#     confidential transfers in one Batch.
if lib_subdir != "win32":
    ffibuilder.cdef(
